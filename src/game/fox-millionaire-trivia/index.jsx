...
const FoxMillionaireTrivia = () => {
  ...
  const lifelines = [
    {
      id: "skip",
      name: "Skip Bayless",
      specialty: "NBA & NFL Expert",
      avatar: "🤔",
      color: "#000000",
      description: "Will eliminate one wrong answer with controversial hot takes",
      helpText: [
        "Listen, I've been saying this for YEARS! Option {option} is absolutely WRONG! I guarantee it!",
        "This is why I'm the most honest analyst in sports! Option {option} is CLEARLY incorrect!"
      ],
      tauntText: [
        "See? This is EXACTLY what I predicted! You should have listened to Uncle Skip!",
        "I TOLD YOU SO! This is why experience matters! You can't just wing it in sports trivia!"
      ],
      encouragementText: [
        "Hey, even Tom Brady has bad games! You'll bounce back stronger than ever!",
        "Don't let the haters get to you! Champions learn from their mistakes!"
      ]
    },
    {
      id: "colin",
      name: "Colin Cowherd",
      specialty: "All Sports Analytics",
      avatar: "📊",
      color: "#333333",
      description: "Provides statistical analysis and narrows down to two best options",
      helpText: [
        "Look at the data here - I'm seeing strong indicators for either {option1} or {option2}. The analytics don't lie.",
        "My research team and I have eliminated {option1} and {option2}. Trust the process and the numbers."
      ],
      tauntText: [
        "The data was RIGHT THERE! Sometimes you have to trust the analytics over your gut feeling.",
        "This is exactly why I focus on metrics and not emotions. The numbers told the story."
      ],
      encouragementText: [
        "Hey, even the best analysts miss sometimes. What matters is learning from the data.",
        "That's sports for you - sometimes the 10% chance happens. Keep trusting the process."
      ]
    },
    {
      id: "joy",
      name: "Joy Taylor",
      specialty: "Breaking News & Insights",
      avatar: "📺",
      color: "#666666",
      description: "Gives insider perspective with 75% accuracy on the best answer",
      helpText: [
        "From my sources and analysis, I'm confident that {option} is your best bet here. Trust me on this one.",
        "Based on everything I'm hearing and seeing, {option} makes the most sense. That's my final answer."
      ],
      tauntText: [
        "Ouch! That one stings! Sometimes you have to go with your first instinct in this business.",
        "Well, that didn't go as planned! But hey, that's why they play the games, right?"
      ],
      encouragementText: [
        "Don't worry about it! Even the best reporters get stories wrong sometimes. You'll get the next one!",
        "That's just part of the game! What matters is coming back stronger and more prepared."
      ]
    }
  ];
  ...
  const questions = [
    ...
    {
      question: "Which quarterback has won the most Super Bowl titles as a starter?",
    ...
    {
      question: "Which university has won the most NCAA Division I men's basketball championships?",
      options: ["Kentucky", "North Carolina", "Duke", "UCLA"],
      correctAnswer: 3
    },
    {
      question: "Who is the all-time leading scorer in NHL history?",
      options: ["Wayne Gretzky", "Gordie Howe", "Jaromír Jágr", "Alex Ovechkin"],
      correctAnswer: 0
    },
    {
    ...
    {
      question: "Which NFL team appeared in four consecutive Super Bowls from 1991-1994, losing all of them?",
      options: ["Minnesota Vikings", "Denver Broncos", "Buffalo Bills", "Atlanta Falcons"],
      correctAnswer: 2
    },
    {
  ...
  React.useEffect(() => {
    let interval = null;
    if (timerRunning && timeLeft > 0 && gameState === "playing" && selectedAnswer === null) {
  ...
  const getLifelineHelp = (lifeline) => {
    ...
    const randomTemplate = helpTemplates[Math.floor(Math.random() * helpTemplates.length)];
    ...
    const secondRandomWrong = wrongAnswers.filter(a => a !== randomWrong)[Math.floor(Math.random() * (wrongAnswers.length - 1))];
    
    // Different logic per lifeline
    if (lifeline.id === "skip") {
      return randomTemplate.replace("{option}", `"${options[randomWrong]}"`);
    } else if (lifeline.id === "colin") {
      if (Math.random() > 0.5) {
    ...
    } else if (lifeline.id === "joy") {
      // 75% chance to suggest correct answer
      if (Math.random() < 0.75) {
  ...
  const renderWelcomeScreen = () => (
    <div className="flex flex-col items-center p-4 w-full">
      <div className="bg-black w-full py-3 mb-6 rounded shadow-lg">
        <h1 className="text-white text-2xl font-bold text-center">
          FOX SPORTS MILLIONAIRE
        </h1>
      </div>
      
      <div className="w-20 h-20 mb-4">
        <Award className="w-full h-full text-gray-800" />
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6 w-full border border-gray-300">
        <h3 className="font-bold mb-2 text-gray-900">Game Rules:</h3>
        <ul className="list-disc pl-6 mb-4 text-sm">
          <li>Answer 14 increasingly difficult sports questions</li>
          <li>Use Fox Sports personalities as lifelines when you need help</li>
      ...
      <h3 className="font-bold mb-2 text-gray-900">Your Fox Sports Lifelines:</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mb-6">
        {lifelines.map(lifeline => (
          ...
          >
            ...
            <p className="text-xs text-gray-600 text-center mt-1">{lifeline.description}</p>
          </div>
        ))}
      </div>
      
      <button 
        className="px-6 py-3 rounded-full text-white text-lg font-bold bg-black hover:bg-gray-800 transition-colors"
        onClick={startGame}
      >
        START GAME
      </button>
  ...
  const renderPlayingScreen = () => {
    ...
    return (
      <div className="flex flex-col items-center p-4 w-full">
        <div className="bg-black w-full py-2 mb-3 rounded shadow-md">
        ...
        <div className="flex justify-between items-center w-full mb-4">
          <div className="text-center bg-gray-100 px-3 py-1 rounded border border-gray-300">
            <TrendingUp className="w-4 h-4 inline mr-1 text-gray-700" />
            <span className="text-sm text-gray-900">{prizeLevels[currentLevel].amount}</span>
          </div>
          
          <div className={`text-center px-3 py-1 rounded font-bold border ${
            timeLeft <= 5 ? 'bg-red-100 text-red-700 animate-pulse border-red-300' : 
            timeLeft <= 10 ? 'bg-yellow-100 text-yellow-700 border-yellow-300' : 'bg-gray-100 text-gray-900 border-gray-300'
          }`}>
            {timeLeft} seconds
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button 
              className="text-sm bg-gray-100 text-gray-700 px-2 py-1 rounded hover:bg-gray-200 border border-gray-300"
        ...
        {/* Question */}
        <div className="bg-gray-50 p-4 rounded mb-4 w-full border border-gray-300">
          <p className="text-center font-bold text-lg text-gray-900">{currentQuestion.question}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 w-full mb-6">
          {currentQuestion.options.map((option, index) => (
            <button
              key={index}
              className={`p-4 border-2 rounded text-left hover:bg-gray-50 transition-colors
            ...
            >
              <span className="font-bold text-gray-800">{["A", "B", "C", "D"][index]}.</span> {option}
            </button>
          ))}
        </div>
            ...
            return (
              <div key={lifeline.id} className="relative">
                <button
                  ...
                  style={{ 
                    backgroundColor: isUsed && !hasRefills
                      ? '#e5e5e5' 
                      : lifeline.color,
                    opacity: isUsed && !hasRefills ? 0.5 : 1
                  }}
                  onClick={() => (!isUsed || hasRefills) && useLifeline(lifeline)}
        ...
        <div className="flex items-center justify-center mb-2">
          ...
          <button 
            className="text-xs bg-black text-white px-2 py-1 rounded flex items-center hover:bg-gray-800"
            onClick={() => setShowingShopModal(true)}
          >
        ...
        {walkAwayConfirm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
            <div className="bg-white p-6 rounded-lg max-w-md mx-4">
              ...
              <p className="mb-4">
                You'll walk away with {prizeLevels[currentLevel].amount}. Are you sure you want to quit now?
              </p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
            <div className="bg-white p-6 rounded-lg w-full max-w-md mx-4">
              ...
              <p className="text-sm mb-4">
                Purchase additional lifelines with tokens to help you reach the million dollars!
              </p>
              
              <div className="space-y-4 mb-6">
                {lifelines.map(lifeline => (
                  <div key={lifeline.id} className="border rounded-lg p-3 flex items-center">
                    <div 
                    ...
                    <div className="flex-grow">
                      <h4 className="font-bold text-sm">{lifeline.name}</h4>
                      <p className="text-xs text-gray-600">{lifeline.description}</p>
                    </div>
                ...
                <div className="border rounded-lg p-3 flex items-center bg-blue-50">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-xs text-gray-600">Get 3 of each lifeline and save 30%!</p>
                  </div>
                  ...
                  <div className="ml-2">
                    <button 
                      ...
                      onClick={() => {
                        lifelines.forEach(l => purchaseLifelineRefill(l.id, 3));
                      }}
                    >
                      <div className="w-4 h-4 mr-1 bg-cyan-400 rounded-full flex items-center justify-center">
                        <div className="w-2 h-2 bg-white transform rotate-45"></div>
                ...
                <div className="border rounded-lg p-3 flex items-center bg-purple-50">
                  ...
                  <div className="flex-grow">
                    <h4 className="font-bold text-sm">Fox Sports+ Subscription</h4>
                    <p className="text-xs text-gray-600">Unlimited lifelines, exclusive questions & more!</p>
                  </div>
  ...
  // Get random ESPN personality reaction
  const getPersonalityReaction = (isWin = false) => {
    const randomPersonality = lifelines[Math.floor(Math.random() * lifelines.length)];
    const reactions = isWin ? randomPersonality.encouragementText : randomPersonality.tauntText;
  ...
  const renderResultsScreen = () => {
    ...
    const winCaption = isCorrect === true && currentLevel === questions.length - 1 
      ? "CONGRATULATIONS! YOU'VE WON A MILLION DOLLARS!" 
      : isCorrect === false 
        ? "GAME OVER" 
        : "YOU WALKED AWAY";
    
    // Get personality reaction for wrong answers
    const personalityReaction = isCorrect === false ? getPersonalityReaction(false) : null;
    ...
    return (
      <div className="flex flex-col items-center p-4 w-full">
        ...
        <div className="bg-gray-100 p-4 rounded-lg mb-6 w-full">
          <p className="text-center">
            {isCorrect === true && currentLevel === questions.length - 1
              ...
              : isCorrect === false
              ? `The correct answer was "${questions[currentLevel].options[questions[currentLevel].correctAnswer]}". Better luck next time!`
              : "You played it safe and walked away with your winnings. Well done!"}
          </p>
        </div>
        
        {/* ESPN Personality Reaction for Wrong Answers */}
        {isCorrect === false && personalityReaction && (
          <div className="bg-white border-2 rounded-lg p-4 mb-6 w-full" style={{ borderColor: personalityReaction.personality.color }}>
            <div className="flex items-start">
              <div 
                className="w-16 h-16 rounded-full flex items-center justify-center text-3xl flex-shrink-0 mr-4"
                style={{ backgroundColor: personalityReaction.personality.color + '20' }}
              >
                {personalityReaction.personality.avatar}
              </div>
              
              <div className="flex-grow">
                <div className="bg-white border rounded-lg p-3 relative">
                  <div className="absolute left-0 top-4 w-3 h-3 bg-white border-l border-t transform rotate-45 -ml-1.5"></div>
                  <div className="font-bold mb-1" style={{ color: personalityReaction.personality.color }}>
                    {personalityReaction.personality.name}
                  </div>
                  <p className="italic text-sm">{personalityReaction.message}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 w-full">
          ...
          <button 
            className="px-4 py-2 rounded bg-gray-600 text-white font-bold hover:bg-gray-700"
            onClick={resetGame}
          >
            MAIN MENU
          </button>
        ...
        {isCorrect === false && (
          <div className="border border-yellow-400 bg-yellow-50 p-3 rounded-lg w-full flex items-center">
            <div className="w-8 h-8 mr-3 bg-cyan-400 rounded-full flex items-center justify-center flex-shrink-0">
              <div className="w-4 h-4 bg-white transform rotate-45"></div>
            </div>
            <div className="flex-grow">
              <h3 className="font-bold text-sm">Want another chance?</h3>
              <p className="text-xs">Purchase extra lifelines with tokens and continue your game where you left off!</p>
            </div>
            <button 
  ...
  // Main render
  return (
    <div className="bg-black min-h-screen flex flex-col items-center pt-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-xl overflow-hidden mb-4 border-2 border-gray-800">
      ...
      <div className="text-sm text-gray-300 flex items-center">
        ...
        <span>Demo version - Fox Sports Millionaire Concept</span>
      </div>
    </div>
  );
};

export default FoxMillionaireTrivia;
